...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("AURION_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("AURION_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("AURION_WORKSPACE_ID")
	agentName := os.Getenv("AURION_AGENT_NAME")
	taskID := os.Getenv("AURION_TASK_ID")
