...
// `no such host` errors when calling out (for example, `aurion issue get`
// hitting the Aurion API). See upstream issue openai/codex#10390.
//
...
// actually reach the Aurion API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Aurion CLI can reach the API.
...
// aurionManagedBeginMarker / aurionManagedEndMarker delimit the block the
...
const (
	aurionManagedBeginMarker = "# BEGIN aurion-managed (do not edit; regenerated by daemon)"
	aurionManagedEndMarker   = "# END aurion-managed"
)

// renderAurionManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.aurion]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertAurionManagedBlock), avoids both traps.
func renderAurionManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(aurionManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(aurionManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(aurionManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(aurionManagedEndMarker) + `\n?`)

// upsertAurionManagedBlock returns the config content with the aurion-managed
...
// `[permissions.aurion]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderAurionManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertAurionManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderAurionManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the aurion-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertAurionManagedBlock(existing, policy)
