...
// checked out on demand by the agent via `aurion repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/aurion_workspaces)
...
// on demand via `aurion repo checkout <url>`.
