...
func BuildPrompt(task Task) string {
	...
	b.WriteString("You are running as a local coding agent for a Aurion workspace.\n\n")
	fmt.Fprintf(&b, "Your assigned issue ID is: %s\n\n", task.IssueID)
	fmt.Fprintf(&b, "Start by running `aurion issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	return b.String()
}
...
func buildChatPrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a chat assistant for a Aurion workspace.\n")
