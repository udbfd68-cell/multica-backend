...
const defaultCLIConfigPath = ".aurion/config.json"
...
// An empty profile returns the default path (~/.aurion/config.json).
// A named profile returns ~/.aurion/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".aurion", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.aurion/. A named profile returns ~/.aurion/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".aurion"), nil
	}
	return filepath.Join(home, ".aurion", "profiles", profile), nil
}
