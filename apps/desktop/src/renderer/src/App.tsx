...
import { workspaceKeys, workspaceListOptions } from "@aurion/core/workspace/queries";
import { api } from "@aurion/core/api";
import { ThemeProvider } from "@aurion/ui/components/common/theme-provider";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.aurion config).
  useEffect(() => {
    window.daemonAPI.setTargetApiUrl(DAEMON_TARGET_API_URL);
  }, []);

  // Listen for invite IDs delivered via deep link (aurion://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (aurion://auth/callback?token=...).
  ...
  useEffect(() => {
    if (!user) return;
    const token = localStorage.getItem("aurion_token");
    if (!token) return;
