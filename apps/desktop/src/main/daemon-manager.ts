...
const PREFS_PATH = join(homedir(), ".aurion", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/aurion/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".aurion", "profiles", profile)
    : join(homedir(), ".aurion");
}
...
// Sidecar file that records which Aurion user the cached PAT in config.json
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["aurion.exe"] : ["aurion"];
 ...
 *   to `apps/desktop/resources/bin/aurion`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Aurion.app>/Contents/Resources/app.asar`.
...
/**
 * Returns a usable `aurion` binary path. Priority:
 ...
 *   5. `aurion` on PATH (dev convenience / user-installed via brew).
 ...
 * `aurion version --output json`. Cached for the process lifetime — the
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Aurion Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, AURION_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "aurion CLI is not installed" };
